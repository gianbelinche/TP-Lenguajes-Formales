...
(defn a-mayusculas-salvo-strings [s] 
  (apply str (map-indexed a-mayusculas-aux (clojure.string/split s #"'")))
)

(defn a-mayusculas-aux [index,s]
  (if (even? index) (.toUpperCase s) (str "'" s "'"))
)
...
(defn ya-declarado-localmente? [ident context]
  (boolean 
      (some (fn [x] (= ident (get x 0)))
          (subvec (get context 1) (last (get context 0)))
    ))
)
...
(defn cargar-var-en-tabla [amb]
  (if (not= :sin-errores (amb 3)) 
      amb
      [(amb 0) 
       (amb 1) 
       (amb 2) 
       (amb 3) 
       (assoc (amb 4) 1 (conj ((amb 4) 1) [(last (amb 2)) (first (amb 2)) (amb 5)]))
       (+ 1 (amb 5)) 
       (amb 6)]
    )
)
...
(defn inicializar-contexto-local [amb]
  (if (not= :sin-errores (amb 3)) 
      ...
      [(amb 0) 
       ...
       (amb 2) 
       (amb 3) 
       (assoc (amb 4) 0 (conj ((amb 4) 0) (count ((amb 4) 1))))
       (amb 5) 
       (amb 6)]
    )
)
...
(defn buscar-coincidencias [amb]
  (filter (fn [x] (= (first x) (last (amb 2)) ))  ((amb 4) 1) )
)
...
(cond
  (not= :sin-errores (amb 3)) amb
  :else 
    (assoc amb 6 (assoc (amb 6) ubi ['JMP (count (amb 6))]) )
)
)
...
(cond
  (not= :sin-errores (amb 3)) amb
  (= operador '<) (assoc amb 6 (conj (amb 6) 'LT))
  (= operador '>) (assoc amb 6 (conj (amb 6) 'GT))
  (= operador '=) (assoc amb 6 (conj (amb 6) 'EQ))
  (= operador '>=) (assoc amb 6 (conj (amb 6) 'GTE))
  (= operador '<=) (assoc amb 6 (conj (amb 6) 'LTE))
  (= operador '<>) (assoc amb 6 (conj (amb 6) 'NEQ))
  :else amb
)
)
